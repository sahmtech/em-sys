...
@section('content')
    <div class="login-form col-md-12 col-xs-12 right-col-content" style="background: white;">
        <img src="/uploads/custom_logo.png" class="img-rounded" alt="Logo"
            style="width: 172px;
            margin: 0px 85px;">

        {{-- <p class="form-header text-white"> </p> --}}
        <h2 class="text-center" style="color: #091e42;margin-top:0px">@lang('lang_v1.helloAgain')</h2>
        <form method="POST" action="{{ route('login') }}" id="login-form" style="padding-top: 10px;">
            {{ csrf_field() }}
            <div class="form-group has-feedback {{ $errors->has('username') ? ' has-error' : '' }}">
                ...
                <input id="username" type="text" class="form-control" name="username" value="{{ $username }}"
                    required autofocus placeholder="@lang('lang_v1.username')" style="border-radius: 3px;padding: 5px;">
            ...
            <div class="form-group has-feedback {{ $errors->has('password') ? ' has-error' : '' }}">
                <input id="password" type="password" class="form-control" name="password" value="{{ $password }}"
                    required placeholder="@lang('lang_v1.password')" style="border-radius: 3px;padding: 5px;">
            ...
            <div class="form-group">
                <div class="checkbox icheck">
                    <label style="color: #091e42;padding: 0px;">
                        <input type="checkbox" name="remember" {{ old('remember') ? 'checked' : '' }}
                            style="color: #091e42"> @lang('lang_v1.remember_me')
                    </label>
                </div>
            </div>
            <br>
            <div class="form-group">
                <button type="submit" class="btn btn-primary btn-flat btn-login"
                    style="    border-radius: 3px;
                width: 100%;">@lang('lang_v1.login')</button>
                @if (config('app.env') != 'demo')
                    <a href="{{ route('password.request') }}" class="pull-left" style="padding-top: 6px;color: #091e42">
