...
class TimeSheetController extends Controller
{
    ...
    public function agentTimeSheetGroups()
    {
        ...
        return DataTables::of($payrolls)
            ->addColumn('action', function ($row) use ($user, $is_admin) {
                ...
                if ($is_admin || $user->can('followup.show_timesheet')) {
                    $html .= '<li><a href="' . route('followup.agentTimeSheet.showTimeSheet', ['id' => $row->id]) . '"><i class="fa fa-eye" aria-hidden="true"></i> ' . __('messages.view') . '</a></li>';
                }
                if ($row->status == 'draft' && ($is_admin || $user->can('followup.edit_timesheet'))) {
                    $html .= '<li><a href="' . route('followup.agentTimeSheet.editTimeSheet', ['id' => $row->id]) . '"><i class="fa fa-edit" aria-hidden="true"></i> ' . __('messages.edit') . '</a></li>';
                }
                if (($is_admin && $row->status == 'final' && $row->is_approved == 0) || ($row->created_by != $user->id && $user->can('followup.deal_timesheet') && $row->status == 'final' && $row->is_approved == 0)) {
                    $html .= '<li><a href="' . route('followup.agentTimeSheet.dealTimeSheet', ['id' => $row->id]) . '"><i class="fa fa-check" aria-hidden="true"></i> ' . __('lang_v1.approve') . '</a></li>';
                }
                $html .= '</ul></div>';
                return $html;
            })
    ...
    public function submitTmeSheet(Request $request)
    {
            ...
            foreach ($payrolls as $payroll) {
                ...
                if ($user) {
                    TimesheetUser::create([
                        ...
                        'work_days' => $payroll['wd'] ?? 30,
                        'absence_days' => $payroll['absence_day'] ?? 0,
                        'absence_amount' => $payroll['absence_amount'] ?? 0,
                        'over_time_hours' => $payroll['over_time_h'] ?? 0,
                        'over_time_amount' => $payroll['over_time'] ?? 0,
                        'other_deduction' => $payroll['other_deduction'] ?? 0,
                        'other_addition' => $payroll['other_addition'] ?? 0,
                        'cost_2' => $payroll['cost2'] ?? 0,
                        'invoice_value' => $payroll['invoice_value'] ?? 0,
                        'vat' => $payroll['vat'] ?? 0,
                        'total' => $payroll['total'] ?? 0,
                        'project_id' => $payroll['assigned_to'] ?? null,
                        'basic' => $payroll['basic'] ?? 0,
                        'housing' => $payroll['housing'] ?? 0,
        ...
        return redirect()->route('followup.agentTimeSheetIndex')->with('status', $output);
    }
