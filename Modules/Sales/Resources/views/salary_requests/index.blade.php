...
$('body').on('click', '.open-edit-modal', function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                $('#editsalaryRequestModal input[name="arrival_period"]').val(data.arrival_period).trigger('change');
...
<script>
    $(document).ready(function () {
        $('#workers_select').on('change', function () {
            ...
            $.ajax({
                url: '{{ route("fetch-worker-details", ["workerId" => ":workerId"]) }}'.replace(':workerId', workerId),
                type: 'GET',
                dataType: 'json',
