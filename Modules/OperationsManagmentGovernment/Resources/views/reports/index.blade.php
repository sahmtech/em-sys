        ...
        <div class="modal fade" id="addReportModal" tabindex="-1" role="dialog">
            <div class="modal-dialog modal-lg" role="document"> <!-- Larger Modal -->
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div class="row">
                            <div class="form-group col-md-4">
                                {!! Form::label('report_contact_id', __('operationsmanagmentgovernment::lang.report_contact') . ':*') !!}
                                {!! Form::select('report_contact_id', $report_contact_id, null, [
                                    'class' => 'form-control select2',
                                    'style' => 'width: 100%;',
                    ...
                    <!-- Last Row: Attachment -->
                    <div class="form-group col-md-12">
                        {!! Form::label('file', __('lang_v1.attachments') . ':') !!}
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var reports_table = $('#reports_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: '{{ route('operationsmanagmentgovernment.reports.index') }}',
                    data: function(d) {
            ...
            $(document).on('click', '.open-edit-modal', function() {
                ...
                $('#addReportForm input[name="_method"]').remove();

                $.ajax({
                    url: '/operationsmanagmentgovernment/reports/edit/' + reportId,
                    type: 'GET',
                    success: function(data) {
                        $('#addReportModal').modal('show');

                        $('#addReportModal .modal-title').text("{{ __('messages.edit') }}");

                        $('#addReportModal button[type=submit]').show();

                        $('#addReportForm').attr('action',
                            '/operationsmanagmentgovernment/reports/' + reportId);

                        $('<input>').attr({
                            type: 'hidden',
                            name: '_method',
                            value: 'PUT'
                        }).appendTo('#addReportForm');

                        $('input, select, textarea').prop('disabled', false);
                        $('#report_type_select').prop('disabled', true);

                        $('#report_type_select').val(data.type).trigger('change');


                        $('#contact_select').val(data.contact_id).trigger('change');
                        $('#date').val(data.date);
                        $('#time').val(data.time);
                        ...
                        if (data.type) {
                            ...
                            $('.' + data.type + '-fields input, .' + data.type +
                                    '-fields select, .' + data.type + '-fields textarea')
                                .prop('required', true);
                        }
                        if (data.file_path) {
            ...
            $(document).on('click', '.open-view-modal', function() {
                ...
                $.ajax({
                    url: '/operationsmanagmentgovernment/reports/view/' + reportId,
                    type: 'GET',
                    success: function(data) {
                        $('#addReportModal').modal('show');
