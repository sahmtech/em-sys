                ...
                <table class="table table-bordered table-striped ajax_view" id="contact_sell_table">
                    ...
                    <tfoot>
                        <tr class="bg-gray font-17 footer-total text-center">
                            ...
                            <td><span class="display_currency" id="footer_total_remaining" data-currency_symbol ="true"></span>
                            </td>
                            <td><span class="display_currency" id="footer_total_sell_return_due"
        ...
        $(document).ready(function() {
            ...
            contact_sell_datatable = $("#contact_sell_table").DataTable({
                ...
                "ajax": {
                    ...
                    "data": function(d) {
                        if ($('#date_range_filter').val()) {
                            var start = $('#date_range_filter').data('daterangepicker').startDate
                                .format('YYYY-MM-DD');
                            var end = $('#date_range_filter').data('daterangepicker').endDate.format(
                                'YYYY-MM-DD');
                            d.start_date = start;
                            d.end_date = end;
                        }
                ...
                "fnDrawCallback": function(oSettings) {
                    ...
                    $('#footer_sale_total').text(sum_table_col($('#contact_sell_table'),
                    'final-total'));

                    $('#footer_total_paid').text(sum_table_col($('#contact_sell_table'), 'total-paid'));

                    $('#footer_total_remaining').text(sum_table_col($('#contact_sell_table'),
                        'payment_due'));

                    $('#footer_total_sell_return_due').text(sum_table_col($('#contact_sell_table'),
                        'sell_return_due'));

                    $('#footer_payment_status_count').html(__sum_status_html($('#contact_sell_table'),
                        'payment-status-label'));

                    $('#service_type_count').html(__sum_status_html($('#contact_sell_table'),
                        'service-type-label'));
                    $('#payment_method_count').html(__sum_status_html($('#contact_sell_table'),
                        'payment-method'));
