...
class IrRequestController extends Controller
{
    ...
    public function index()
    {
        ...
        $users = User::whereIn('id', $userIds)->where('status', '!=', 'inactive')->where('user_type', 'worker')->select('id', DB::raw("CONCAT(COALESCE(first_name, ''),' ',COALESCE(last_name,''), ' - ',COALESCE(id_proof_number,'')) as full_name"))->pluck('full_name', 'id');
