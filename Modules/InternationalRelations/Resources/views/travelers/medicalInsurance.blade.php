...
@section('content')
    ...
    <section class="content">
        @include('internationalrelations::layouts.nav_trevelers')
        ...
        $(document).ready(function() {
            ...
            $('#medicalExamination_table').on('change', '.select-row', function() {
                $('#select-all').prop('checked', $('.select-row:checked').length ===
                    medicalExamination_table.rows().count());
            });
    ...
    <script>
        ...
        function viewInsurance(workerId) {
            $.ajax({
                ...
                error: function() {
                    alert(
                    'Failed to retrieve insurance information.'); // Optionally, handle this more gracefully
                }
            });
            ...
            if (companyId) {
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        $('#classSelect').empty();
                        $('#classSelect').append('<option value="">' +
                            '{{ __('essentials::lang.insurance_class') }}' + '</option>');

                        $.each(data, function(key, value) {
                            $('#classSelect').append('<option value="' + key + '">' + value +
                                '</option>');
                        });
                    }
                });
