        ...
        if (request()->ajax()) {
            $asset_allocated = AssetTransaction::join(
                ...
                ->select(
                    ...
                    'asset_transactions.transaction_datetime as revoked_at',
                    'asset_transactions.id as id',
                    'assets.name as asset',
                    'assets.model as model',
                    'CAT.name as category',
                    DB::raw("CONCAT(COALESCE(receiver.surname, ''),' ',COALESCE(receiver.first_name, ''),' ',COALESCE(receiver.last_name,'')) as revoked_for"),
