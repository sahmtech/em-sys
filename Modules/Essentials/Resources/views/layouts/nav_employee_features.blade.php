            ...
            <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
                <ul class="nav navbar-nav">
                    ...
                    @if (auth()->user()->hasRole('Admin#1') || auth()->user()->can('essentials.crud_employee_features'))
                        <li @if (request()->segment(2) == 'featureIndex') class="active" @endif><a
                                href="{{ route('featureIndex') }}">@lang('essentials::lang.allowances')</a></li>
                    @endif

                    @if (auth()->user()->hasRole('Admin#1') || auth()->user()->can('essentials.curd_travel_categories_features'))
                        <li @if (request()->segment(2) == 'userTravelCat') class="active" @endif><a
                                href="{{ route('userTravelCat') }}">@lang('essentials::lang.travel_categories')</a></li>
                    @endif
