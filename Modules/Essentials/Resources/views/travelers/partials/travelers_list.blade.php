        ...
        <tr>
            <td colspan="5">
                <div style="display: flex; width: 100%;">
                    {!! Form::open([
                        'url' => action([\Modules\Essentials\Http\Controllers\TravelersController::class, 'postarrivaldata']),
                        'method' => 'post',
                        'id' => 'arrived_form',
                    ]) !!}

                    {!! Form::hidden('selected_rows', null, ['id' => 'selected_rows']) !!}
                    @include('essentials::travelers.partials.border_arrival_modal')

                    {{-- @if (auth()->user()->hasRole('Admin#1') || auth()->user()->can('change_arrived_status')) --}}
                    @if (auth()->user()->hasRole('Admin#1') ||
                            auth()->user()->can('essentials.import_new_arrival_workers') ||
                            auth()->user()->can('essentials.import_new_arrival_workers'))
                        {!! Form::submit(__('essentials::lang.arrived'), [
