        ...
        @component('components.widget', ['class' => 'box-primary'])
            <div class="table-responsive">
                <div style="margin-bottom: 10px;">
                    @if (auth()->user()->hasRole('Admin#1') || auth()->user()->can('essentials.change_request_status'))
                        <button type="button" class="btn btn-warning change_status2">
                            @lang('request.change_status')
                        </button>
                    @endif
                    @if (auth()->user()->hasRole('Admin#1') || auth()->user()->can('essentials.return_request'))
    ...
    <script type="text/javascript">
        $(document).ready(function() {
            ...
            var requests_table = $('#requests_table').DataTable({
                ...
                serverSide: true,
                ajax: {
                    url: "{{ route('essentials.getFilteredRequests', ['filter' => ':filter']) }}"
