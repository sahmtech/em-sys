            ...
            ->addColumn(
                ...
                function ($row) use ($sale_type) {
                    ...
                    if (auth()->user()->can("sell.view") || auth()->user()->can("direct_sell.view") || auth()->user()->can("view_own_sell_only")) {
                        $html .= '<a href="#" data-href="' . url("sells/" . $row->id) . '" class="btn-modal btn btn-info btn-xs" data-container=".view_modal"><i class="fas fa-eye" aria-hidden="true"></i> ' . __("messages.view") . '</a>';
                    }
                    ...
                    if (auth()->user()->can('accounting.map_transactions')) {
                        ...
                        if (\request()->from == 'journal') {
                            if (!$is_mapped) {
                                $html .= '<a href="#" 
                                    data-href="' . action("\Modules\Accounting\Http\Controllers\JournalEntryController@map") . '?id=' . $row->id . '&type=sell' . '" class="btn-modal btn btn-primary btn-xs mt-10" data-container=".view_modal"><i class="fas fa-link"></i> ' . __('accounting::lang.map_transaction') . '</a>';
                            } else {
                                $html .= '<a href="#" 
                                    data-href="' . action("\Modules\Accounting\Http\Controllers\JournalEntryController@map") . '?id=' . $row->id . '&type=sell' . '" class="btn-modal btn btn-warning btn-xs mt-10" data-container=".view_modal"><i class="fas fa-link"></i> ' . __('accounting::lang.edit_mapping') . '</a>';
                            }
                        } else {
                            if (!$is_mapped) {
                                $html .= '<a href="#" 
            ...
            ->addColumn('transaction_number', function ($row) {
                ...
                if ($row->type == 'sell') {
                    $html = '<button type="button" class="btn btn-link btn-modal"
                                data-href="' . url("sells/" . $row->transaction_id) . '" data-container=".view_modal">' . $row->invoice_no . '</button>';
                } elseif ($row->type == 'purchase') {
                    $html = '<button type="button" class="btn btn-link btn-modal"
                                data-href="' . url("purchases/" . $row->transaction_id) . '" data-container=".view_modal">' . $row->ref_no . '</button>';
                }
                return $html;
            })
