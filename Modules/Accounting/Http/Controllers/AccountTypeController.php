...
class AccountTypeController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            $query = AccountingAccountType::where('account_type', request()->input('account_type'))
                ...
                ->with('parent')
                ->select(['name', 'description', 'id', 'business_id', 'company_id', 'parent_id', 'account_primary_type']);
            ...
            return Datatables::of($query)
                ...
                ->addColumn('parent_type', function ($row) {
                    if (!empty($row->parent_id)) {
                        if (empty($row->company_id) && !empty($row->description)) {
                ...
                ->addColumn(
                    'action',
                    '@if(!empty($company_id))<button data-href="{{action(\'\Modules\Accounting\Http\Controllers\AccountTypeController@edit\', [$id])}}" class="btn btn-xs btn-primary btn-modal" data-container="#edit_account_type_modal"><i class="glyphicon glyphicon-edit"></i> @lang("messages.edit")</button>
                        &nbsp;
    ...
    public function update(Request $request, $id)
    {
        ...
        try {
            ...
            $account_type = AccountingAccountType::where('business_id', $business_id)->where('company_id', $company_id)
                ->find($id);
