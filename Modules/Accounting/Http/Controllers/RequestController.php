...
class RequestController extends Controller
{
    ...
    public function index()
    {
        ...
        $can_accounting_requests_change_status = auth()->user()->can('accounting.change_status');
        ...
        if (request()->ajax()) {
            return DataTables::of($requestsProcess ?? [])
                ...
                ->editColumn('status', function ($row)  use ($is_admin, $can_accounting_requests_change_status) {
                    $status = '';
                    $procedureStart=EssentialsWkProcedure::where('id',$row->procedure_id)->first();
                    if($procedureStart->start != 1){
                        if ($is_admin || $can_accounting_requests_change_status) {
        ...
        return view('accounting::requests.allRequest')->with(compact('users','requestTypes' ,'statuses', 'main_reasons', 'classes', 'leaveTypes'));
    }
