<table class="table table-bordered table-striped">
    ...
    <tbody>
        @foreach ($accounts as $account)
            <tr class="bg-gray">
                <td>
                    <div class="btn-group"><button type="button" class="btn btn-info dropdown-toggle btn-xs"
                            data-toggle="dropdown" aria-expanded="false">{{ __('messages.actions') }}<span
            ...
            @if (count($account->child_accounts) > 0)
                @foreach ($account->child_accounts as $child_account)
                    <tr>
                        ...
                        <td>{{ $account->name }}</td>
                        <td>
                            @if (!empty($child_account->account_primary_type))
                                {{ __('accounting::lang.' . $child_account->account_primary_type) }}
                            @endif
                        </td>
                        <td>
                            @if (!empty($child_account->account_sub_type))
                                {{ __('accounting::lang.' . $child_account->account_sub_type->name) }}
                            @endif
                        </td>
                        <td>
                            @if (!empty($child_account->detail_type))
                                {{ __('accounting::lang.' . $child_account->detail_type->name) }}
                            @endif
                        </td>
                        <td>
